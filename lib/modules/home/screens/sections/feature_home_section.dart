...
class FeatureHomeSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<FeatureBloc, FeatureState>(
      builder: (context, state) {
        ...
        } else if (state is FeatureFailed) {
  ...
  Widget _buildWidgetFailed(BuildContext context) {
    return ErrorPlaceholder(
      ...
      onTap: () {
        context.read<FeatureBloc>().add(FeatureFetching());
      },
    );
  }

  Widget _buildWidgetSuccess(FeatureLoaded state, BuildContext context) {
    return Padding(
      padding: const EdgeInsets.symmetric(
        horizontal: 24.0,
      ),
      child: Row(
        ...
        children: List.generate(
          ...
          (index) {
            ...
            return GestureDetector(
              onTap: () {
                context.navigateTo(PageRouteInfo(state.feature[index].route,
                    path: state.feature[index].path));
              },
              child: _buildFeatureContainer(
                state.feature[index],
